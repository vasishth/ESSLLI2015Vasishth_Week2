...
\title{Lecture 2: Statistical methods for linguistic research: Advanced Tools}
...
\begin{enumerate}
\item
Give you a feeling for how Bayesian analysis works using two simple examples that need only basic mathematical operations (addition, subtraction, division, multiplication). 
\item 
...
I will assign some homework which is designed to help you understand these concepts. Solutions will be discussed next week.
...
We will use this fact in Homework 0.\label{hw0hint}
\end{frame}
...
\begin{equation}
\log f(x_1)+ \log f(x_2) \times \dots \times \log f(x_10)
\end{equation}
...
\item
Suppose we have 46 successes out of 100. We generally use the empirically observed proportion 46/100 as our estimate of $\theta$. I.e., we assume that the \textbf{generating distribution} is
...
\begin{enumerate}
\item
If we don't have much prior information, we could use a=b=2; this gives us a uniform prior; we will call this a vague prior. 
\item
...
So: the posterior mean is a \textbf{weighted mean} of the prior and likelihood.
...
I'll leave this as Homework 0 (solution next week). Hint: see slide~\pageref{hw0hint}.
...
\begin{equation}
\mu[i] \sim \beta_0 + \beta_1  x_i \quad \hbox{Note: predictor is centered}
\end{equation}
...
\item $\sim$ can be read as ``is generated or modeled by''
\item %\texttt{$<$-}
\begin{verbatim}
<-
\end{verbatim}
...
\begin{frame}[fragile]\frametitle{Homework 1: Rats' weights}
...
\begin{frame}[fragile]\frametitle{Homework 2: Rats' weights (solution in next class)}
...
We fit log reading time to Trial id (centered), Native Language, and Sex. The categorical variables are centered as well. Note: This model is incorrect! We should have fit a linear mixed model.
...
Here is the lm fit for comparison (I centered all predictors):
...
\item Coming up next: Linear mixed models.
\end{enumerate}
